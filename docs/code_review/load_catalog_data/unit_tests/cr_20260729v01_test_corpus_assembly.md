---
name: cr_20260729v01_test_corpus_assembly
goal: Address code quality issues identified in code/load_catalog_data/unit_tests/test_corpus_assembly.py to align with python-development (unit-tests, type-hints, comments, docstrings) skills.
created: 2026-07-29 14:37:44
updated: 2026-07-29 14:37:44
---

## Implementation Plan

1. [completed] Tighten loose substring assertions - `code/load_catalog_data/unit_tests/test_corpus_assembly.py`
...
2. [completed] Define shared helpers before first use - `code/load_catalog_data/unit_tests/test_corpus_assembly.py`
   - 2.1. [suggestion] Line 703 / 725: `test_tables_missing_or_blank_description_rejected` and `test_columns_missing_or_blank_description_rejected` call `_path_id(...)`, but that helper is defined later at line 851 (under the "Document-shape guards" banner). It works because pytest resolves the name at run time, but forward references make the file harder to follow top-to-bottom.
        - Current: `_path_id` first used at line 703, defined at line 851
        - Expected: move `_path_id` (and its `_path_id`-adjacent doc) above its earliest use, or relocate the two description tests to sit after the helper's definition
...
1. unit-tests: No issues found — file/function naming follow `test_<module>` / `test_<function>_<scenario>_<expected>`; pytest is used throughout with `@pytest.mark.parametrize`, `pytest.raises`, and `tmp_path`; tests are order-independent (input-order independence is explicitly asserted in `test_assemble_corpus_issue_order_independent_of_input_order`); assertions target the public `AssemblyError.issues` contract, not private state.
2. type-hints: No issues found — every helper, fixture, test, and nested function has fully typed parameters and return annotations, using modern syntax (`list[str]`, `str | None`, `dict[str, Any]`). The single `# type: ignore[arg-type]` in `_path_id` is a justified narrowing to the `file_type` Literal.
3. comments: No issues found — inline comments consistently explain the "why" (the behavior/rule under test), matching the pytest convention of documenting intent per test.
4. docstrings: No issues found — the module and all shared helpers carry docstrings; per standard pytest practice the test functions rely on descriptive names plus intent comments rather than docstrings.
